// JavaScript Document
var nTotalNum=10;
var sType=decodeURIComponent(getWinUrl('typeId'));
...
function searchContent(){
	...
	if(appVue.searchList){
		...
		$.ajax({
			url:'/asg/portal/h5/sortlist.html?totalNum='+nTotalNum+'&typeId='+sType+'&index='+appVue.pageIndex+'&name='+sTypeName,
			type:'get',
			//dataType:"json",
			success: function(data){
				appVue.loadingShow=false;
				appVue.show=true;
				//fPushState('search_classify_type.html?totalNum='+nTotalNum+'&index='+appVue.pageIndex+'&typeId='+decodeURI(sType)+'&name='+decodeURI(sTypeName));
				//console.log(data)
				data=jsonStr(data);
				appVue.searchIndex=data['map']['index'];
				appVue.titleName=sTypeName;
				if(data['map']['booksList'].length<=0 && data['map']['index']==1){
					//第一页就没有数据
					appVue.searchList=false;
					appVue.scrollPage=false;
					appVue.searchMain=[];
				...
				//搜索不到东西了
				if(data['map']['booksList'].length<nTotalNum && data['map']['index']>=1){
					appVue.scrollPage=false;
					appVue.loadingShow=true;
