// JavaScript Document
var nTotalNum=100;
var sType=decodeURIComponent(getWinUrl('id'));
...
function searchContent(){
	...
	if(appVue.searchList){
		...
		$.ajax({
			url:'/asg/portal/h5/rankList.html?totalNum='+nTotalNum+'&id='+sType+'&index='+appVue.pageIndex+'&name='+sTypeName,
			type:'get',
			//dataType:"json",
			success: function(data){
				appVue.loadingShow=false;
				appVue.show=true;
				//fPushState('ranking_list.html?totalNum='+nTotalNum+'&index='+appVue.pageIndex+'&id='+decodeURI(sType)+'&name='+decodeURI(sTypeName));
				//console.log(data)
				data=jsonStr(data);
